...
var mediaConstraints = {
  ...
  video: {
    aspectRatio: {
      // https://developer.mozilla.org/zh-CN/docs/Web/API/MediaTrackConstraints#aspectratio
      // 设置宽高比
...
async function createPeerConnection() {
  ...
  myPeerConnection = new RTCPeerConnection({
    // 如果未指定，则将在没有可用的 STUN 或 TURN 服务器的情况下进行连接尝试，这将连接限制为本地对等点。
    iceServers: [
  ...
  myPeerConnection.onnegotiationneeded = handleNegotiationNeededEvent; // 通过向连接添加轨道，将触发此事件，从而导致需要重新协商连接。
...
async function handleNegotiationNeededEvent() {
  ...
  try {
    ...
    if (myPeerConnection.signalingState != "stable") {
      // 描述连接或重新连接到另一个对等方时连接本地端的信令进程的状态
...
function handleUserlistMsg(msg) {
  ...
  msg.users.forEach(function (username) {
    ...
    item.addEventListener("click", shareScreen, false);
...
async function invite(evt) {
  ...
  if (myPeerConnection) {
    ...
    try {
      webcamStream
        .getTracks() // 返回流中所有的MediaStreamTrack列表，比如音频轨道和视频轨道
        .forEach(
          (transceiver = (track) =>
            // addTransceiver() 方法创建一个新的 RTCRtpTransceiver 并将其添加到与 RTCPeerConnection 关联的收发器集合中。每个收发器代表一个双向流，具有与其关联的 RTCRtpSender 和 RTCRtpReceiver.
...
async function handleVideoOfferMsg(msg) {
  ...
  var desc = new RTCSessionDescription(msg.sdp); // 指示它所描述的请求/应答协商过程的SDP 协议的相关描述
  ...
  await myPeerConnection.setLocalDescription(
    await myPeerConnection.createAnswer() // 创建一个新的 RTCSessionDescription 对象，包含对本地描述的应答
  );
...
async function shareScreen(evt) {
  var clickedUsername = evt.target.textContent;
  ...
  try {
    ...
    webcamStream
      .getTracks() // 返回流中所有的MediaStreamTrack列表，比如音频轨道和视频轨道
      .forEach((track) => myPeerConnection.addTrack(track, webcamStream));
  } catch (err) {
