...
/**
  * @type {Array<{username: string, clientID: number, connected: boolean, sendUTF: (arg0: string) => void}>}
  [{
    username: string, // 发送端用户名
    clientID: number,
    connected: boolean,
    sendUTF: (arg0: string) => void
  }]
 */
var connectionArray = []; // 连接对象数组
var nextID = Date.now();
...
// 初始化 WebSocket 服务器
// 文档: https://github.com/theturtle32/WebSocket-Node/blob/master/docs/WebSocketServer.md
function initWsServer() {
  var wsServer = new WebSocketServer({
    httpServer: webServer,
    autoAcceptConnections: false, // 每当发出新的 WebSocket 请求时，服务器都会发出request事件
  });
  ...
  wsServer.on("request", function (request) {
    ...
    // webSocketRequest.accept('chosen-protocol', 'accepted-origin')
    // 返回值包含已建立的 WebSocketConnection 实例
    var connection = request.accept("json", request.origin); // https://github.com/theturtle32/WebSocket-Node/blob/master/docs/WebSocketRequest.md
...
initHTTPS();
initWebServer(); // Start the server
