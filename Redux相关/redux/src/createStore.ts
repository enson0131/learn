...
>(
  reducer: Reducer<S, A, PreloadedState>,
  preloadedState?: PreloadedState | StoreEnhancer<Ext, StateExt> | undefined, // 初始化 state
  ...
  // 如果第二个参数是函数，第三个参数是 undefined，那么就把第二个参数 当做中间件
  ...
  let currentState: S | PreloadedState | undefined = preloadedState as
    | PreloadedState
    | undefined // 记录当前的 state
  let currentListeners: Map<number, ListenerCallback> | null = new Map() // 记录订阅的事件
  let nextListeners = currentListeners // nextListeners 是 currentListeners 的快照
   ...
   */
  // 这个方法用于确认 nextListeners 是 currentListeners 的快照， 而不是同一个引用
  ...
  function getState(): S {
    ...
    return currentState as S // 通过 getState 获取当前的 state
  }
